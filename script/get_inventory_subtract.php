...
if (isset($pdo) && $pdo !== null) {
    try {
        ...
        // Add condition to exclude "For Approval" items for inventory_out.php
        $whereClauses[] = "i.inventory_status != 'For Approval'";
        ...
        // Add condition to exclude "For Approval" items in the filtered count as well
        $filteredWhereClauses = [];
        
        // Add condition to exclude "For Approval" items
        $filteredWhereClauses[] = "i.inventory_status != 'For Approval'";
        
        if ($warehouse_id && $user_role !== 'Warehouse Admin') {
            $filteredWhereClauses[] = "i.warehouse_id = :warehouse_id";
        }

        if (!empty($searchValue)) {
            $filteredWhereClauses[] = "(it.item_name LIKE :search OR w.warehouse_name LIKE :search OR i.qty LIKE :search OR i.inventory_id LIKE :search OR i.inventory_status LIKE :search)";
        }

        if (!empty($filteredWhereClauses)) {
            $filteredQuery .= " WHERE " . implode(" AND ", $filteredWhereClauses);
        }
        ...
        // Column mapping based on inventory_out.php DataTables configuration:
        // 0: Inventory ID
        // 1: Warehouse
        // 2: Item
