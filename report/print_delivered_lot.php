<?php 
    ...
    if (isset($_GET['export']) && $_GET['export'] === 'csv') {
        ...
        header('Content-Disposition: attachment; filename=delivered_by_lot_' . date('Y-m-d') . '.csv');
        
        $output = fopen('php://output', 'w');
        fwrite($output, "\xEF\xBB\xBF");
        
        // CSV Headers
        fputcsv($output, ['Lot Name', 'Total Deliveries', 'Delivered', 'Pending', 'Accepted', 'Delivery Rate (%)']);
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4>🚚 Delivered by Lot Report <?= $selectedProject > 0 ? "- " . htmlspecialchars($selectedProjectName) : "" ?></h4>
...
<table id="deliveredLotTable" class="table table-bordered shadow-sm">
...
function printDeliveryReport() {
    printTable(
        'deliveredLotTable', 
        'Delivered Deliveries by Lot Report', 
